...
TXN_QUERIES = {
    ...
    "PAYMENT": {
        ...
        "updateBCCustomer": "UPDATE CUSTOMER SET data = jsonb_set(data, '{C_BALANCE, C_YTD_PAYMENT, C_PAYMENT_CNT, C_DATA}', jsonb_build_array(to_jsonb(%s::numeric), to_jsonb(%s::numeric), to_jsonb(%s::integer), to_jsonb(%s)), true) WHERE (data->'C_W_ID')::smallint = %s AND (data->'C_D_ID')::smallint = %s AND (data->'C_ID')::integer = %s",
